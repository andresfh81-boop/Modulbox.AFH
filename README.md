# Configurador MODULBOX-400 (PWA offline)

App instalable en el móvil (PWA) para configurar soportes, brazos y generar el plano técnico de toldos MODULBOX-400. Funciona sin conexión una vez instalada. Disponible en 6 idiomas (ES/EN/FR/DE/IT/PT).
...
Cada vez que cambies `index.html`, sube los cambios (`git add -A && git commit -m "..." && git push`) y sube en 1 el número de `CACHE_NAME` en `sw.js` (p. ej. `modulbox-v2`) para que los móviles descarguen la versión nueva.
